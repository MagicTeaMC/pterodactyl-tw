...
@section('content-header')
    <h1>{{ $node->name }}<small>配置你的節點設置.</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li class="active"><a href="{{ route('admin.nodes.view.settings', $node->id) }}">設置</a></li>
...
<form action="{{ route('admin.nodes.view.settings', $node->id) }}" method="POST">
    <div class="row">
                ...
                <div class="box-body row">
                    <div class="form-group col-xs-12">
                        ...
                        <div>
                            ...
                            <p class="text-muted"><small>字元限制: <code>a-zA-Z0-9_.-</code> 和 <code>[空格]</code> (最少 1, 最多 100 字元).</small></p>
                        </div>
                    </div>
                    <div class="form-group col-xs-12">
                    ...
                    <div class="form-group col-xs-12">
                        <label for="public" class="control-label">允許自動超額分配 <sup><a data-toggle="tooltip" data-placement="top" title="允許自動分配給該節點？">?</a></sup></label>
                        <div>
                    ...
                    <div class="form-group col-xs-12">
                        <label for="fqdn" class="control-label">功能變數名稱</label>
                        <div>
                            <input type="text" autocomplete="off" name="fqdn" class="form-control" value="{{ old('fqdn', $node->fqdn) }}" />
                        </div>
                        <p class="text-muted"><small>請輸入用於連接守護程式的功能變數名稱 (例如 <code>node.example.com</code>). 僅當您沒有為此節點使用 SSL 時才可以使用 IP 位址.
                                <a tabindex="0" data-toggle="popover" data-trigger="focus" title="為什麼需要(FQDN)功能變數名稱？" data-content="為了保護您的伺服器與此節點之間的通信，我們需要使用 SSL。我們無法為 IP 位址生成 SSL 證書，因此您需要提供(FQDN)功能變數名稱。">為什麼？</a>
                            </small></p>
                    </div>
                    <div class="form-group col-xs-12">
                        ...
                        <p class="text-muted small">在大多數情況下，您應該選擇使用 SSL 連接。如果使用 IP 位址或者您根本不想使用 SSL，請選擇 HTTP 連接.</p>
                    </div>
                    <div class="form-group col-xs-12">
                        <label class="form-label"><span class="label label-warning"><i class="fa fa-power-off"></i></span> 通過代理</label>
                        <div>
                            <div class="radio radio-success radio-inline">
                    ...
                    <div class="form-group col-xs-12">
                        ...
                        <div>
                            <div class="radio radio-success radio-inline">
                                ...
                                <label for="pMaintenanceFalse"> 關閉</label>
                            </div>
                            <div class="radio radio-warning radio-inline">
                                ...
                                <label for="pMaintenanceTrue"> 開啟</label>
                            </div>
                        </div>
                        <p class="text-muted small">如果節點被標記為“維護中”，用戶將無法訪問該節點上的伺服器.</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="box-body row">
                    <div class="col-xs-12">
                        <div class="row">
                            ...
                            <div class="form-group col-xs-6">
                                <label for="memory_overallocate" class="control-label">超額分配百分比</label>
                                <div class="input-group">
                        ...
                        <p class="text-muted small">輸入此節點上可用於分配給伺服器的記憶體總量。您還可以提供一個百分比，允許分配超過定義的記憶體.</p>
                    </div>
                    <div class="col-xs-12">
                        <div class="row">
                            <div class="form-group col-xs-6">
                                <label for="disk" class="control-label">總存儲空間</label>
                                <div class="input-group">
                            ...
                            <div class="form-group col-xs-6">
                                <label for="disk_overallocate" class="control-label">超額分配百分比</label>
                                <div class="input-group">
                        ...
                        <p class="text-muted small">輸入此節點上可用於伺服器分配存儲空間總量。您還可以提供一個百分比來確定超出設置限制的存儲空間量以允許.</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-6">
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">常規配置</h3>
                </div>
                <div class="box-body row">
                    <div class="form-group col-xs-12">
                        <label for="disk_overallocate" class="control-label">最大網頁文件上傳大小</label>
                        <div class="input-group">
                            <input type="text" name="upload_size" class="form-control" value="{{ old('upload_size', $node->upload_size) }}"/>
                            <span class="input-group-addon">MiB</span>
                        </div>
                        <p class="text-muted"><small>輸入可以通過基於 Web 的檔案管理員上傳的檔的最大大小.</small></p>
                    </div>
                    <div class="col-xs-12">
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="daemonListen" class="control-label"><span class="label label-warning"><i class="fa fa-power-off"></i></span> 守護進程埠</label>
                                <div>
                                    <input type="text" name="daemonListen" class="form-control" value="{{ old('daemonListen', $node->daemonListen) }}"/>
                                </div>
                            </div>
                            <div class="form-group col-md-6">
                                <label for="daemonSFTP" class="control-label"><span class="label label-warning"><i class="fa fa-power-off"></i></span> 守護進程 SFTP 埠</label>
                                <div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <p class="text-muted"><small>守護進程運行自己的 SFTP 管理容器，不使用主物理伺服器上的 SSHd 進程. <Strong>不要使用為物理伺服器的 SSH 進程分配的相同埠.</strong></small></p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-xs-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">保存設置</h3>
                </div>
                <div class="box-body row">
                    <div class="form-group col-sm-6">
                        <div>
                            <input type="checkbox" name="reset_secret" id="reset_secret" /> <label for="reset_secret" class="control-label">重置守護進程超級金鑰</label>
                        </div>
                        <p class="text-muted"><small>重置守護程式超級金鑰將使來自舊金鑰的任何請求無效。此金鑰用於守護程式上的所有敏感操作，包括伺服器創建和刪除。我們建議定期更改此金鑰以確保安全.</small></p>
                    </div>
                </div>
