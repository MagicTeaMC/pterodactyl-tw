...
const StartupContainer = () => {
    ...
    return !data ? (
        !error || (error && isValidating) ? (
            ...
            <ServerError title={'糟糕!'} message={httpErrorToHuman(error)} onRetry={() => mutate()} />
        )
    ) : (
        <ServerContentBlock title={'啟動設定'} showFlashKey={'startup:image'}>
                ...
                <TitledGreyBox title={'Docker 映像'} css={tw`flex-1 lg:flex-none lg:w-1/3 mt-8 md:mt-0 md:ml-10`}>
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        ...
                        <>
                            ...
                            {isCustomImage && (
                                <p css={tw`text-xs text-neutral-300 mt-2`}>
                                    此 {"server's"} Docker 映像檔由管理員手動設定舞法在這裡更改
                                </p>
                            )}
                        </>
                    )}
            ...
            <h3 css={tw`mt-8 mb-2 text-2xl`}>參數</h3>
